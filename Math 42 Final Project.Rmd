...
diag(friendship_levels) <- 0 # relationships with oneself are set to 0
```
...
ind_event_probs <- list(
  ...
  unenjoyable_activity = 0.1, # go down by 1
...
ggplot(history_df, aes(x = Time, y = population_mean)) +
  ...
  labs(
    ...
    title = "Population Mean Happiness Over Two Years"
  ) +
  theme_minimal() +
